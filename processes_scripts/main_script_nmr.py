...
def main(training, apply_sparsity):
    ...
    lambda_h_j_dec = 1e-5       # Scalar for H_j_dec sparsity term
    p = 2.                      # Error power
    ...
    # Defining objectives
    rec_criterion = loss_functions.p_power_error                 # Reconstruction criterion
    ...
    scheduler = RedLR(optimizer, 'min', factor=0.5, patience=3, verbose=True)
    if training:
        ...
        for epoch in range(epochs):
            ...
            for index in range(totTrainFiles / numFilesPerTr):
                ...
                # Precomputed inverse masking threshold
                ...
                for batch in tqdm(range(ms.shape[0] / B)):
                    ...
                    loss = torch.log(((loss + loss_mask) * Variable(torch.from_numpy(
                        mt[batch * B: (batch + 1) * B, L:-L, :]).cuda())).mean() + 1e-6)
                    ...
                    # Sparsity term
                    if apply_sparsity:
                        sparsity_penalty = H_j_dec.norm(1.) * lambda_h_j_dec
            ...
            if (epoch + 1) % 50 == 0:
                print('------------   Saving model   ------------')
                torch.save(encoder.state_dict(), 'results/torch_sps_encoder_l2_log' + str(epoch + 1) + '.pytorch')
                torch.save(decoder.state_dict(), 'results/torch_sps_decoder_l2_log' + str(epoch + 1) + '.pytorch')
                torch.save(sp_decoder.state_dict(), 'results/torch_sps_sp_decoder_l2_log' + str(epoch + 1) + '.pytorch')
                torch.save(source_enhancement.state_dict(), 'results/torch_sps_se_l2_log' + str(epoch + 1) + '.pytorch')
                print('------------       Done       ------------')
    else:
